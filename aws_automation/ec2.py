...
def start_instance(ec2_client,instance_id):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidInstanceID.NotFound':
            ...
            log.error(f"ClientError while starting instance: {e.response['Error']['Message']}")
...
def stop_instance(ec2_client,instance_id):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidInstanceID.NotFound':
            ...
            log.error(f"ClientError while stopping instance: {e.response['Error']['Message']}")
    except BotoCoreError as e:
...
def terminate_instance(ec2_client,instance_id):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidInstanceID.NotFound':
            ...
            log.error(f"ClientError while terminating instance: {e.response['Error']['Message']}")
    except BotoCoreError as e:
...
def list_running_instances(ec2_client):
    ...
    try:
        ...
        log.error(f"ClientError while listing instances: {e.response['Error']['Message']}")
        return []
    except BotoCoreError as e:
